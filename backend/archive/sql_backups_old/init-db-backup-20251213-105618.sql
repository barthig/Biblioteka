...
CREATE TABLE loan (
    id INT NOT NULL DEFAULT nextval('loan_id_seq'),
    book_id INT NOT NULL,
...
CREATE TABLE fine (
    id INT NOT NULL DEFAULT nextval('fine_id_seq'),
...
-- Użytkownicy: administrator, bibliotekarz, czytelnik
-- Hasło dla wszystkich kont testowych: admin
INSERT INTO app_user (
    id, email, name, roles, password, phone_number, address_line, city, postal_code,
    blocked, verified, verified_at, pending_approval, membership_group, loan_limit,
    blocked_reason, created_at, updated_at, privacy_consent_at, newsletter_subscribed
) VALUES
    (1, 'admin@example.com', 'Administrator', '["ROLE_ADMIN"]'::json, '$2y$10$917zt1OKnnhXTN8/kOA/yeyLp3CUXfprQJGh3oLLzDWL32cTtXJtm',
        '+48 500 100 200', 'Ul. Biblioteczna 1', 'Warszawa', '00-001', false, true, now(), false, 'standard', 10, NULL, now(), now(), now(), true),
    (2, 'bibliotekarz@biblioteka.local', 'Barbara Bibliotekarka', '["ROLE_LIBRARIAN"]'::json, '$2y$10$917zt1OKnnhXTN8/kOA/yeyLp3CUXfprQJGh3oLLzDWL32cTtXJtm',
        '+48 500 200 300', 'Ul. Czytelników 5', 'Kraków', '30-001', false, true, now(), false, 'standard', 8, NULL, now(), now(), now(), true),
    (3, 'marta.nowak@example.com', 'Marta Nowak', '["ROLE_USER"]'::json, '$2y$10$917zt1OKnnhXTN8/kOA/yeyLp3CUXfprQJGh3oLLzDWL32cTtXJtm',
        '+48 500 300 400', 'Ul. Lipowa 12/4', 'Poznań', '60-012', false, true, now(), false, 'standard', 5, NULL, now(), now(), now(), true);
SELECT setval('app_user_id_seq', 3, true);

-- Autorzy i kategorie
INSERT INTO author (id, name) VALUES
    (1, 'Olga Tokarczuk'),
    (2, 'Zygmunt Miłoszewski'),
    (3, 'Marek Krajewski');
SELECT setval('author_id_seq', 3, true);

INSERT INTO category (id, name) VALUES
    (1, 'Literatura piękna'),
    (2, 'Kryminał'),
    (3, 'Reportaż');
SELECT setval('category_id_seq', 3, true);

-- Książki wraz z kategoriami
...
VALUES
    (1, 1, 'Księgi Jakubowe', '9788308058902', 3, 3, 1, 2, 'Szeroka epicka opowieść historyczna', 'Wydawnictwo Literackie', 2014, 'print', 'LIT-001', 'adult', now()),
    (2, 2, 'Uwikłanie', '9788374145258', 2, 2, 1, 1, 'Pierwszy tom serii z prokuratorem Szackim', 'W.A.B.', 2007, 'print', 'KRY-045', 'adult', now()),
    (3, 3, 'Mock', '9788380693419', 2, 2, 1, 1, 'Prequel serii o Eberhardzie Mocku', 'Znak', 2016, 'print', 'KRY-078', 'adult', now());
...
-- Egzemplarze w inwentarzu
INSERT INTO book_copy (id, book_id, inventory_code, status, location, access_type, condition_state, created_at, updated_at) VALUES
    (1, 1, 'INV-00001', 'loaned', 'Oddział Główny', 'open', 'dobry', now(), now()),
    (2, 1, 'INV-00002', 'available', 'Oddział Główny', 'open', 'dobry', now(), now()),
    (3, 2, 'INV-00003', 'reserved', 'Filia Śródmieście', 'open', 'bardzo dobry', now(), now()),
    (4, 3, 'INV-00004', 'available', 'Filia Śródmieście', 'open', 'dobry', now(), now());
SELECT setval('book_copy_id_seq', 4, true);

-- Plik cyfrowy (okładka / PDF próbny)
INSERT INTO book_digital_asset (id, book_id, label, original_filename, mime_type, size, storage_name, created_at) VALUES
    (1, 1, 'Okładka', 'ksiegi-jakubowe.jpg', 'image/jpeg', 204800, 'covers/ksiegi-jakubowe.jpg', now());
SELECT setval('book_digital_asset_id_seq', 1, true);

-- Aktywne wypożyczenie i rezerwacja
INSERT INTO loan (id, book_id, book_copy_id, user_id, borrowed_at, due_at, returned_at, extensions_count, last_extended_at) VALUES
    (1, 1, 1, 3, now() - interval '7 days', now() + interval '14 days', NULL, 0, NULL);
SELECT setval('loan_id_seq', 1, true);

INSERT INTO reservation (id, book_id, book_copy_id, user_id, status, reserved_at, expires_at, fulfilled_at, cancelled_at) VALUES
    (1, 2, 3, 3, 'active', now() - interval '1 day', now() + interval '5 days', NULL, NULL);
SELECT setval('reservation_id_seq', 1, true);

INSERT INTO fine (id, loan_id, amount, currency, reason, created_at, paid_at) VALUES
    (1, 1, 12.50, 'PLN', 'Przetrzymanie egzemplarza', now() - interval '1 day', NULL);
SELECT setval('fine_id_seq', 1, true);

-- Role i ustawienia systemowe
INSERT INTO staff_role (id, name, role_key, modules, description, created_at, updated_at) VALUES
    (1, 'Bibliotekarz', 'ROLE_LIBRARIAN', '["circulation","catalog","reports"]'::json, 'Dostęp do modułów wypożyczeń i katalogu', now(), now()),
    (2, 'Administrator', 'ROLE_ADMIN', '["admin","integrations","catalog","circulation"]'::json, 'Pełne uprawnienia systemowe', now(), now());
SELECT setval('staff_role_id_seq', 2, true);

INSERT INTO system_setting (id, setting_key, setting_value, value_type, description, created_at, updated_at) VALUES
    (1, 'library.name', 'Miejska Biblioteka Publiczna', 'string', 'Nazwa wyświetlana w panelu', now(), now()),
    (2, 'library.address', 'Ul. Biblioteczna 1, 00-001 Warszawa', 'string', 'Adres głównej siedziby', now(), now());
SELECT setval('system_setting_id_seq', 2, true);

INSERT INTO integration_config (id, name, provider, enabled, settings, last_status, last_tested_at, created_at, updated_at) VALUES
    (1, 'Poczta elektroniczna', 'smtp', true, '{"host":"smtp.example.com","port":587,"from":"powiadomienia@biblioteka.local"}'::json, 'ok', now(), now(), now());
SELECT setval('integration_config_id_seq', 1, true);

INSERT INTO backup_record (id, file_name, file_path, file_size, status, created_at, initiated_by) VALUES
    (1, 'bootstrap.sql', '/backups/bootstrap.sql', 1024, 'done', now(), 'system');
SELECT setval('backup_record_id_seq', 1, true);

INSERT INTO notification_log (id, user_id, type, channel, fingerprint, payload, status, error_message, sent_at) VALUES
    (1, 3, 'loan_reminder', 'email', 'loan-1-due', '{"loanId":1,"dueInDays":14}'::json, 'sent', NULL, now());
SELECT setval('notification_log_id_seq', 1, true);

INSERT INTO audit_logs (id, user_id, entity_type, entity_id, action, ip_address, old_values, new_values, description, created_at) VALUES
    (1, 1, 'book', 1, 'create', '127.0.0.1', NULL, '{"title":"Księgi Jakubowe"}', 'Utworzenie rekordu książki', now());
SELECT setval('audit_logs_id_seq', 1, true);

INSERT INTO weeding_record (id, book_id, book_copy_id, processed_by_id, reason, action, condition_state, notes, removed_at) VALUES
    (1, 3, 4, 2, 'Egzemplarz zniszczony', 'discard', 'zużyty', 'Usunięto po audycie rocznym', now());
SELECT setval('weeding_record_id_seq', 1, true);
