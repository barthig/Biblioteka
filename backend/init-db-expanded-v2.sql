...
-- ============================================
-- PeÄąâ€šna struktura bazy danych systemu bibliotecznego
-- Data aktualizacji: 2025-12-13 12:00
-- System: Biblioteka - System ZarzĂ„â€¦dzania BibliotekĂ„â€¦
-- Wersja: 2.1.0
-- 
-- FUNKCJONALNOĹšCI:
-- - System ocen ksiÄ…ĹĽek (rating)
-- - System feedbacku rekomendacji (recommendation_feedback)
-- - Kolekcje kuratorskie (book_collection)
-- - Onboarding uÄąÄ˝ytkownika (preferred_categories)
-- - SzczegÄ‚Ĺ‚Äąâ€šowy profil uÄąÄ˝ytkownika (pesel, card_number, address)
-- - System kar finansowych (fine)
-- - Audyt aktywnoÄąâ€şci (audit_logs)
-- - WypoÄąÄ˝yczenia z przedÄąâ€šuÄąÄ˝eniami (loan)
...
-- Tabela: staff_role - Role pracownikow
...
-- Tabela: app_user - UÄąÄ˝ytkownicy systemu
CREATE TABLE app_user (
    ...
    avatar_storage_name VARCHAR(255) DEFAULT NULL,
    avatar_mime_type VARCHAR(100) DEFAULT NULL,
    avatar_updated_at TIMESTAMP(0) WITHOUT TIME ZONE DEFAULT NULL,
    PRIMARY KEY(id)
);
...
-- Tabela: author - Autorzy ksiĂ„â€¦ÄąÄ˝ek
CREATE TABLE author (
    id INT GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    name VARCHAR(255) NOT NULL,
    PRIMARY KEY(id)
...
-- Tabela: category - Kategorie ksiĂ„â€¦ÄąÄ˝ek
...
-- Tabela: age_range - PrzedziaÄąâ€šy wiekowe (do rekomendacji)
...
-- Tabela: book_category - Relacja wiele do wielu miĂ„â„˘dzy ksiĂ„â€¦ÄąÄ˝kami a kategoriami
...
-- Tabela: book_copy - Egzemplarze ksiĂ„â€¦ÄąÄ˝ek
...
-- Tabela: book_digital_asset - Zasoby cyfrowe ksiĂ„â€¦ÄąÄ˝ek
...
-- Tabela: rating - Oceny ksiĂ„â€¦ÄąÄ˝ek
CREATE TABLE rating (
    id INT GENERATED BY DEFAULT AS IDENTITY NOT NULL,
...
-- Tabela: recommendation_feedback - Feedback uÄąÄ˝ytkownika na rekomendacje
...
-- Tabela: collection_books - Relacja wiele do wielu miĂ„â„˘dzy kolekcjami a ksiĂ„â€¦ÄąÄ˝kami
...
-- ============================================
-- WYPOÄąÂ»YCZENIA I REZERWACJE
-- ============================================

-- Tabela: loan - WypoÄąÄ˝yczenia
CREATE TABLE loan (
    id INT GENERATED BY DEFAULT AS IDENTITY NOT NULL,
...
-- ============================================
-- MODUÄąÂ UÄąÂ»YTKOWNIKÄ‚â€śW
-- ============================================

-- Tabela: favorite - Ulubione ksiĂ„â€¦ÄąÄ˝ki uÄąÄ˝ytkownikÄ‚Ĺ‚w
...
-- ============================================
-- MODUÄąÂ ADMINISTRACYJNY
-- ============================================

-- Tabela: announcement - OgÄąâ€šoszenia
...
-- Tabela: notification_log - Logi powiadomieÄąâ€ž
...
-- Tabela: acquisition_budget - BudÄąÄ˝ety akwizycji
...
-- Tabela: acquisition_order - ZamÄ‚Ĺ‚wienia akwizycyjne
...
-- Tabela: weeding_record - Rekordy selekcji zbiorÄ‚Ĺ‚w
...
ALTER TABLE weeding_record ADD CONSTRAINT FK_WR_BOOK FOREIGN KEY (book_id) REFERENCES book (id) NOT DEFERRABLE INITIALLY IMMEDIATE;
...
ALTER TABLE weeding_record ADD CONSTRAINT FK_WR_PROCESSED_BY FOREIGN KEY (processed_by_id) REFERENCES app_user (id) NOT DEFERRABLE INITIALLY IMMEDIATE;
...
-- PrzedziaÄąâ€šy wiekowe (do profilu czytelnika i dopasowania do book.target_age_group)
INSERT INTO age_range (code, label, min_age, max_age, description) VALUES
('child',  'Dzieci (0Ă˘â‚¬â€ś12)', 0, 12, 'Czytelnicy dzieciĂ„â„˘cy'),
('teen',   'MÄąâ€šodzieÄąÄ˝ (13Ă˘â‚¬â€ś17)', 13, 17, 'Czytelnicy mÄąâ€šodzieÄąÄ˝owi'),
('adult',  'DoroÄąâ€şli (18Ă˘â‚¬â€ś64)', 18, 64, 'Czytelnicy doroÄąâ€şli'),
...
(4, 'user04@example.com', 'Maria Czytelnik', '["ROLE_USER"]', '$2y$13$Xhz8P0vQ.zKL9XGkYPfcReZE8QYj1MJOvGk.W7nRZ7I8x5YqzLVnm', '+48821834232', 'ul. Czytelnicza 4', 'KrakÄ‚Ĺ‚w', '93-572', '91022225867', 'CARD-00004', false, false, NULL, false, 'student', 'teen', 10, NULL, NOW() - INTERVAL '83 days', NOW(), NOW(), true, '[16,29,30]', false),
(5, 'user05@example.com', 'Piotr WiÄąâ€şniewski', '["ROLE_USER"]', '$2y$13$Xhz8P0vQ.zKL9XGkYPfcReZE8QYj1MJOvGk.W7nRZ7I8x5YqzLVnm', NULL, 'ul. Czytelnicza 5', 'GdaÄąâ€žsk', '83-455', '73102218531', 'CARD-00005', false, true, NOW() - INTERVAL '22 days', false, 'premium', 'teen', 15, NULL, NOW() - INTERVAL '506 days', NOW(), NOW(), true, '[2,5,20,28]', true),
(6, 'user06@example.com', 'Katarzyna WÄ‚Ĺ‚jcik', '["ROLE_USER"]', '$2y$13$Xhz8P0vQ.zKL9XGkYPfcReZE8QYj1MJOvGk.W7nRZ7I8x5YqzLVnm', '+48892770813', 'ul. Czytelnicza 6', 'WrocÄąâ€šaw', '46-569', '70061949249', 'CARD-00006', false, true, NOW() - INTERVAL '9 days', false, 'standard', 'teen', 5, NULL, NOW() - INTERVAL '448 days', NOW(), NOW(), false, '[8,27]', true),
(7, 'user07@example.com', 'Tomasz Kowalczyk', '["ROLE_USER"]', '$2y$13$Xhz8P0vQ.zKL9XGkYPfcReZE8QYj1MJOvGk.W7nRZ7I8x5YqzLVnm', '+48658946996', 'ul. Czytelnicza 7', 'PoznaÄąâ€ž', '24-823', '98092567526', 'CARD-00007', false, true, NOW() - INTERVAL '65 days', false, 'student', 'teen', 10, NULL, NOW() - INTERVAL '85 days', NOW(), NOW(), true, '[3,10,29]', false),
(8, 'user08@example.com', 'Agnieszka KamiÄąâ€žska', '["ROLE_USER"]', '$2y$13$Xhz8P0vQ.zKL9XGkYPfcReZE8QYj1MJOvGk.W7nRZ7I8x5YqzLVnm', '+48888233424', 'ul. Czytelnicza 8', 'ÄąÂÄ‚Ĺ‚dÄąĹź', '72-711', '96102021622', 'CARD-00008', false, false, NULL, true, 'premium', 'teen', 15, NULL, NOW() - INTERVAL '336 days', NOW(), NOW(), true, '[6,12,22,23]', true),
(9, 'user09@example.com', 'PaweÄąâ€š Lewandowski', '["ROLE_USER"]', '$2y$13$Xhz8P0vQ.zKL9XGkYPfcReZE8QYj1MJOvGk.W7nRZ7I8x5YqzLVnm', '+48753195262', 'ul. Czytelnicza 9', 'Szczecin', '66-858', '71041513474', 'CARD-00009', false, true, NOW() - INTERVAL '67 days', false, 'standard', 'teen', 5, NULL, NOW() - INTERVAL '398 days', NOW(), NOW(), true, '[2,24]', true),
(10, 'user10@example.com', 'Magdalena ZieliÄąâ€žska', '["ROLE_USER"]', '$2y$13$Xhz8P0vQ.zKL9XGkYPfcReZE8QYj1MJOvGk.W7nRZ7I8x5YqzLVnm', NULL, 'ul. Czytelnicza 10', 'Katowice', '80-973', '76012173587', 'CARD-00010', false, true, NOW() - INTERVAL '106 days', false, 'student', 'teen', 10, NULL, NOW() - INTERVAL '438 days', NOW(), NOW(), true, '[4,20,24]', false),
(11, 'user11@example.com', 'MichaÄąâ€š SzymaÄąâ€žski', '["ROLE_USER"]', '$2y$13$Xhz8P0vQ.zKL9XGkYPfcReZE8QYj1MJOvGk.W7nRZ7I8x5YqzLVnm', '+48643743859', 'ul. Czytelnicza 11', 'Lublin', '76-711', '72062832251', 'CARD-00011', false, true, NOW() - INTERVAL '90 days', false, 'premium', 'child', 15, NULL, NOW() - INTERVAL '277 days', NOW(), NOW(), true, '[7,14,21,29]', true),
(12, 'user12@example.com', 'Joanna WoÄąĹźniak', '["ROLE_USER"]', '$2y$13$Xhz8P0vQ.zKL9XGkYPfcReZE8QYj1MJOvGk.W7nRZ7I8x5YqzLVnm', '+48738967621', 'ul. Czytelnicza 12', 'BiaÄąâ€šystok', '28-285', '77010525065', 'CARD-00012', false, false, NULL, false, 'standard', 'child', 5, NULL, NOW() - INTERVAL '394 days', NOW(), NOW(), false, '[13,16]', true),
(13, 'user13@example.com', 'Krzysztof DĂ„â€¦browski', '["ROLE_USER"]', '$2y$13$Xhz8P0vQ.zKL9XGkYPfcReZE8QYj1MJOvGk.W7nRZ7I8x5YqzLVnm', '+48733021385', 'ul. Czytelnicza 13', 'Warszawa', '75-624', '81021169893', 'CARD-00013', false, true, NOW() - INTERVAL '11 days', false, 'student', 'child', 10, NULL, NOW() - INTERVAL '517 days', NOW(), NOW(), true, '[23,25,29]', false),
(14, 'user14@example.com', 'Monika KozÄąâ€šowska', '["ROLE_USER"]', '$2y$13$Xhz8P0vQ.zKL9XGkYPfcReZE8QYj1MJOvGk.W7nRZ7I8x5YqzLVnm', '+48541486973', 'ul. Czytelnicza 14', 'KrakÄ‚Ĺ‚w', '23-707', '94021169073', 'CARD-00014', false, true, NOW() - INTERVAL '67 days', false, 'premium', 'child', 15, NULL, NOW() - INTERVAL '562 days', NOW(), NOW(), true, '[1,3,14,28]', true),
(15, 'user15@example.com', 'ÄąÂukasz Jankowski', '["ROLE_USER"]', '$2y$13$Xhz8P0vQ.zKL9XGkYPfcReZE8QYj1MJOvGk.W7nRZ7I8x5YqzLVnm', NULL, 'ul. Czytelnicza 15', 'GdaÄąâ€žsk', '31-878', '98122640219', 'CARD-00015', false, true, NOW() - INTERVAL '55 days', false, 'standard', 'child', 5, NULL, NOW() - INTERVAL '575 days', NOW(), NOW(), true, '[8,11]', true),
(16, 'user16@example.com', 'Natalia Mazur', '["ROLE_USER"]', '$2y$13$Xhz8P0vQ.zKL9XGkYPfcReZE8QYj1MJOvGk.W7nRZ7I8x5YqzLVnm', '+48611554339', 'ul. Czytelnicza 16', 'WrocÄąâ€šaw', '78-849', '97122486675', 'CARD-00016', false, false, NULL, true, 'student', 'adult', 10, NULL, NOW() - INTERVAL '389 days', NOW(), NOW(), true, '[14,16,21]', false),
(17, 'user17@example.com', 'Marcin Krawczyk', '["ROLE_USER"]', '$2y$13$Xhz8P0vQ.zKL9XGkYPfcReZE8QYj1MJOvGk.W7nRZ7I8x5YqzLVnm', '+48719544823', 'ul. Czytelnicza 17', 'PoznaÄąâ€ž', '62-257', '79041013866', 'CARD-00017', true, true, NOW() - INTERVAL '74 days', false, 'premium', 'adult', 15, 'NieopÄąâ€šacone kary', NOW() - INTERVAL '547 days', NOW(), NOW(), true, '[10,27]', true),
(18, 'user18@example.com', 'Aleksandra Piotrowska', '["ROLE_USER"]', '$2y$13$Xhz8P0vQ.zKL9XGkYPfcReZE8QYj1MJOvGk.W7nRZ7I8x5YqzLVnm', '+48741997838', 'ul. Czytelnicza 18', 'ÄąÂÄ‚Ĺ‚dÄąĹź', '14-419', '77040652408', 'CARD-00018', false, true, NOW() - INTERVAL '120 days', false, 'standard', 'adult', 5, NULL, NOW() - INTERVAL '444 days', NOW(), NOW(), false, '[12,15]', true),
...
(22, 'user22@example.com', 'Karolina KrÄ‚Ĺ‚l', '["ROLE_USER"]', '$2y$13$Xhz8P0vQ.zKL9XGkYPfcReZE8QYj1MJOvGk.W7nRZ7I8x5YqzLVnm', '+48502692841', 'ul. Czytelnicza 22', 'BiaÄąâ€šystok', '83-811', '81082853114', 'CARD-00022', false, true, NOW() - INTERVAL '114 days', false, 'student', 'adult', 10, NULL, NOW() - INTERVAL '243 days', NOW(), NOW(), true, '[3,10,15]', false),
(23, 'user23@example.com', 'RafaÄąâ€š Wieczorek', '["ROLE_USER"]', '$2y$13$Xhz8P0vQ.zKL9XGkYPfcReZE8QYj1MJOvGk.W7nRZ7I8x5YqzLVnm', '+48735884072', 'ul. Czytelnicza 23', 'Warszawa', '93-747', '89030884195', 'CARD-00023', false, true, NOW() - INTERVAL '106 days', false, 'premium', 'adult', 15, NULL, NOW() - INTERVAL '452 days', NOW(), NOW(), true, '[10,15,19,25]', true),
(24, 'user24@example.com', 'Weronika JabÄąâ€šoÄąâ€žska', '["ROLE_USER"]', '$2y$13$Xhz8P0vQ.zKL9XGkYPfcReZE8QYj1MJOvGk.W7nRZ7I8x5YqzLVnm', '+48564787143', 'ul. Czytelnicza 24', 'KrakÄ‚Ĺ‚w', '90-342', '76040435843', 'CARD-00024', false, false, NULL, true, 'standard', 'adult', 5, NULL, NOW() - INTERVAL '239 days', NOW(), NOW(), false, '[5,12]', true),
(25, 'user25@example.com', 'Mateusz Nowicki', '["ROLE_USER"]', '$2y$13$Xhz8P0vQ.zKL9XGkYPfcReZE8QYj1MJOvGk.W7nRZ7I8x5YqzLVnm', NULL, 'ul. Czytelnicza 25', 'GdaÄąâ€žsk', '11-761', '78020223882', 'CARD-00025', false, true, NOW() - INTERVAL '91 days', false, 'student', 'adult', 10, NULL, NOW() - INTERVAL '549 days', NOW(), NOW(), true, '[12,17,30]', false),
(26, 'user26@example.com', 'Julia Maj', '["ROLE_USER"]', '$2y$13$Xhz8P0vQ.zKL9XGkYPfcReZE8QYj1MJOvGk.W7nRZ7I8x5YqzLVnm', '+48894061181', 'ul. Czytelnicza 26', 'WrocÄąâ€šaw', '21-720', '82050739503', 'CARD-00026', false, true, NOW() - INTERVAL '100 days', false, 'premium', 'adult', 15, NULL, NOW() - INTERVAL '413 days', NOW(), NOW(), true, '[12,15,25,29]', true),
(27, 'user27@example.com', 'Bartosz Ostrowski', '["ROLE_USER"]', '$2y$13$Xhz8P0vQ.zKL9XGkYPfcReZE8QYj1MJOvGk.W7nRZ7I8x5YqzLVnm', '+48508008844', 'ul. Czytelnicza 27', 'PoznaÄąâ€ž', '47-274', '99112326556', 'CARD-00027', false, true, NOW() - INTERVAL '91 days', false, 'standard', 'adult', 5, NULL, NOW() - INTERVAL '196 days', NOW(), NOW(), true, '[6,28]', true),
(28, 'user28@example.com', 'Ewa GÄ‚Ĺ‚rska', '["ROLE_USER"]', '$2y$13$Xhz8P0vQ.zKL9XGkYPfcReZE8QYj1MJOvGk.W7nRZ7I8x5YqzLVnm', '+48505094687', 'ul. Czytelnicza 28', 'ÄąÂÄ‚Ĺ‚dÄąĹź', '86-571', '80012533403', 'CARD-00028', false, false, NULL, false, 'student', 'senior', 10, NULL, NOW() - INTERVAL '15 days', NOW(), NOW(), true, '[9,14,24]', false),
(29, 'user29@example.com', 'Patryk Kubiak', '["ROLE_USER"]', '$2y$13$Xhz8P0vQ.zKL9XGkYPfcReZE8QYj1MJOvGk.W7nRZ7I8x5YqzLVnm', '+48738559209', 'ul. Czytelnicza 29', 'Szczecin', '82-774', '82041017608', 'CARD-00029', true, true, NOW() - INTERVAL '73 days', false, 'premium', 'senior', 15, 'NieopÄąâ€šacone kary', NOW() - INTERVAL '428 days', NOW(), NOW(), true, '[7,9,20]', true),
...
(2, 'Olga Tokarczuk'),
(3, 'StanisÄąâ€šaw Lem'),
(4, 'Ryszard KapuÄąâ€şciÄąâ€žski'),
(5, 'WisÄąâ€šawa Szymborska'),
(6, 'Henryk Sienkiewicz'),
(7, 'CzesÄąâ€šaw MiÄąâ€šosz'),
(8, 'J.K. Rowling'),
(9, 'George Orwell'),
(10, 'Isaac Asimov'),
(11, 'Stephen King'),
...
(23, 'Frank Herbert'),
(24, 'Albert Camus'),
(25, 'Gabriel GarcÄ‚Â­a MÄ‚Ë‡rquez'),
(26, 'Umberto Eco'),
(27, 'Dan Brown'),
(28, 'Jo NesbÄąâ„˘'),
...
-- KsiĂ„â€¦ÄąÄ˝ki (30)
INSERT INTO book (id, title, isbn, author_id, copies, total_copies, storage_copies, open_stack_copies, description, publisher, publication_year, resource_type, signature, target_age_group, created_at, updated_at) VALUES
(1, 'Wiedzmin: Ostatnie zyczenie', '978-83-7469-627-4', 1, 5, 5, 3, 2, 'Zbior opowiadan wprowadzajacy postac Geralta z Rivii.', 'SuperNOWA', 1993, 'Ksiazka drukowana', 'F-SAP-001', 'adult', NOW(), NOW()),
(2, 'Ksiega Jakubowe', '978-83-08-06384-6', 2, 3, 3, 2, 1, 'Epicka opowiesc o Jakubie Franku i jego wspolnocie.', 'Wydawnictwo Literackie', 2014, 'Ksiazka drukowana', 'L-TOK-001', 'adult', NOW(), NOW()),
(3, 'Solaris', '978-83-7506-127-3', 3, 4, 4, 2, 2, 'Klasyczna powiesc SF o niepoznawalnej inteligencji planety.', 'Wydawnictwo Literackie', 1961, 'Ksiazka drukowana', 'SF-LEM-001', 'adult', NOW(), NOW()),
(4, 'Imperium', '978-83-240-0942-7', 4, 2, 2, 1, 1, 'Reportaz o rozpadzie ZSRR i podrozy po jego peryferiach.', 'Czytelnik', 1993, 'Ksiazka drukowana', 'R-KAP-001', 'adult', NOW(), NOW()),
(5, 'Sto pociech', '978-83-08-03371-9', 5, 2, 2, 2, 0, 'Zbior wierszy noblistki z lat 60. XX wieku.', 'Wydawnictwo a5', 1967, 'Ksiazka drukowana', 'P-SZY-001', 'adult', NOW(), NOW()),
(6, 'Quo Vadis', '978-83-240-1234-5', 6, 6, 6, 4, 2, 'Powiesc historyczna z czasow Nerona i pierwszych chrzescijan.', 'Czytelnik', 1896, 'Ksiazka drukowana', 'H-SIE-001', 'teen', NOW(), NOW()),
(7, 'Zniewolony umysl', '978-83-08-04567-8', 7, 3, 3, 2, 1, 'Esej o postawach intelektualistow wobec totalitaryzmu.', 'Instytut Literacki', 1953, 'Ksiazka drukowana', 'E-MIL-001', 'adult', NOW(), NOW()),
(8, 'Harry Potter i Kamien Filozoficzny', '978-83-7327-823-4', 8, 8, 8, 4, 4, 'Poczatek przygod mlodego czarodzieja w Hogwarcie.', 'Media Rodzina', 1997, 'E-book', 'F-ROW-001', 'child', NOW(), NOW()),
(9, 'Rok 1984', '978-83-240-5678-9', 9, 5, 5, 3, 2, 'Dystopia o spoleczenstwie pod wszechobecna kontrola.', 'Muza', 1949, 'Ksiazka drukowana', 'SF-ORW-001', 'adult', NOW(), NOW()),
(10, 'Fundacja', '978-83-7469-234-5', 10, 4, 4, 2, 2, 'Pierwszy tom cyklu o psychohistorii i upadku imperium.', 'Zysk i S-ka', 1951, 'Ksiazka drukowana', 'SF-ASI-001', 'teen', NOW(), NOW()),
(11, 'Mistrz i Malgorzata', '978-83-1409-859-4', 11, 5, 5, 4, 1, 'Satyryczna powiesc z watkiem diabelskiej wizyty w Moskwie.', 'Czytelnik', 1967, 'Ksiazka drukowana', 'SIG-11-011', 'adult', NOW(), NOW()),
(12, 'Zbrodnia i kara', '978-83-2679-792-8', 12, 3, 3, 0, 3, 'Psychologiczna powiesc o winie, karze i odkupieniu.', 'Wydawnictwo Literackie', 1866, 'Ksiazka drukowana', 'SIG-12-012', 'adult', NOW(), NOW()),
(13, 'Duma i uprzedzenie', '978-83-1488-195-3', 13, 5, 5, 1, 4, 'Klasyka literatury obyczajowej o rodzinie Bennetow.', 'Penguin Classics', 1813, 'E-book', 'SIG-13-013', 'teen', NOW(), NOW()),
(14, 'Gra o tron', '978-83-1434-674-3', 14, 10, 10, 4, 6, 'Pierwszy tom sagi o walce o wladze w Westeros.', 'Zysk i S-ka', 1996, 'Audiobook', 'SIG-14-014', 'adult', NOW(), NOW()),
(15, 'Diuna', '978-83-8359-703-4', 15, 2, 2, 2, 0, 'Epicka powiesc SF o planecie Arrakis i walce rodow.', 'Rebis', 1965, 'Ksiazka drukowana', 'SIG-15-015', 'teen', NOW(), NOW()),
(16, 'American Gods', '978-83-6574-384-2', 16, 5, 5, 3, 2, 'Opowiesc o starych i nowych bogach we wspolczesnej Ameryce.', 'Mag', 2001, 'E-book', 'SIG-16-016', 'adult', NOW(), NOW()),
(17, 'Opowiesc podrecznej', '978-83-2519-489-1', 17, 7, 7, 2, 5, 'Antyutopia o spoleczenstwie Gilead i kontroli nad kobietami.', 'Zysk i S-ka', 1985, 'Ksiazka drukowana', 'SIG-17-017', 'adult', NOW(), NOW()),
(18, 'Lsnienie', '978-83-5333-926-0', 18, 9, 9, 1, 8, 'Thriller o hotelu Overlook i narastajacym obledzie.', 'Albatros', 1977, 'Ksiazka drukowana', 'SIG-18-018', 'adult', NOW(), NOW()),
(19, 'Morderstwo w Orient Expressie', '978-83-7201-180-8', 19, 6, 6, 0, 6, 'Kryminal z Herkulesem Poirot i zagadka w pociagu.', 'Wydawnictwo Dolnoslaskie', 1934, 'E-book', 'SIG-19-019', 'teen', NOW(), NOW()),
(20, 'Norwegian Wood', '978-83-6925-691-3', 20, 3, 3, 3, 0, 'Nostalgiczna opowiesc o dorastaniu w Tokio lat 60.', 'Muza', 1987, 'Audiobook', 'SIG-20-020', 'adult', NOW(), NOW()),
(21, 'Sto lat samotnosci', '978-83-5741-181-3', 21, 3, 3, 0, 3, 'Saga rodu Buendia i magiczny realizm Macondo.', 'Wydawnictwo Literackie', 1967, 'Ksiazka drukowana', 'SIG-21-021', 'adult', NOW(), NOW()),
(22, 'Imie rozy', '978-83-8428-750-5', 22, 4, 4, 2, 2, 'Intryga kryminalna w sredniowiecznym klasztorze.', 'Noir Sur Blanc', 1980, 'E-book', 'SIG-22-022', 'adult', NOW(), NOW()),
(23, 'Kod Leonarda da Vinci', '978-83-4432-786-4', 23, 3, 3, 2, 1, 'Thriller o tajemnicy zakonnej i szyfrach sztuki.', 'Sonia Draga', 2003, 'Audiobook', 'SIG-23-023', 'teen', NOW(), NOW()),
(24, 'Czlowiek z Wysokiego Zamku', '978-83-5010-267-7', 24, 8, 8, 4, 4, 'Alternatywna historia swiata po zwyciestwie osi.', 'Rebis', 1962, 'Ksiazka drukowana', 'SIG-24-024', 'adult', NOW(), NOW()),
(25, 'Wladca Pierscieni: Druzyna Pierscienia', '978-83-4598-801-5', 25, 2, 2, 2, 0, 'Pierwszy tom trylogii o Wladcy Pierscieni.', 'Muza', 1954, 'E-book', 'SIG-25-025', 'teen', NOW(), NOW()),
(26, 'Kosmos', '978-83-6168-510-4', 26, 3, 3, 2, 1, 'Groteskowa powiesc o poszukiwaniu sensu i znakow.', 'Wydawnictwo Literackie', 1965, 'Ksiazka drukowana', 'SIG-26-026', 'adult', NOW(), NOW()),
(27, 'Sapiens: Od zwierzat do bogow', '978-83-6155-317-7', 27, 8, 8, 1, 7, 'Historia czlowieka od pradziejow po wspolczesnosc.', 'PWN', 2011, 'Ksiazka drukowana', 'SIG-27-027', 'adult', NOW(), NOW()),
(28, 'Kondycja ludzka', '978-83-5339-242-3', 28, 10, 10, 2, 8, 'Powiesc o rewolucji w Szanghaju i dylematach moralnych.', 'Gallimard', 1933, 'E-book', 'SIG-28-028', 'adult', NOW(), NOW()),
(29, 'Rok 1000', '978-83-8019-697-6', 29, 7, 7, 4, 3, 'Popularnonaukowy portret Europy u progu drugiego milenium.', 'PWN', 1989, 'Audiobook', 'SIG-29-029', 'adult', NOW(), NOW()),
(30, 'Psychologia tlumu', '978-83-9348-605-1', 30, 2, 2, 2, 0, 'Klasyczne studium zachowan zbiorowych i mechanizmow mas.', 'PWN', 1895, 'Ksiazka drukowana', 'SIG-30-030', 'adult', NOW(), NOW());

-- Relacje ksiĂ„â€¦ÄąÄ˝ka-kategoria (30)
...
-- Dodatkowe kategorie dla ksiĂ„â€¦ÄąÄ˝ek (Äąâ€şrednio 2Ă˘â‚¬â€ś4 kategorie na tytuÄąâ€š)
INSERT INTO book_category (book_id, category_id) VALUES
(1, 5),
(2, 8),
(2, 30),
...
(2, 1, 'INV-001-002', 'AVAILABLE', 'PÄ‚Ĺ‚Äąâ€ška gÄąâ€šÄ‚Ĺ‚wna', 'OPEN_STACK', 'Bardzo dobry', NOW(), NOW()),
(3, 1, 'INV-001-003', 'BORROWED', 'WypoÄąÄ˝yczona', 'OPEN_STACK', 'Dobry', NOW(), NOW()),
(4, 2, 'INV-002-001', 'AVAILABLE', 'Magazyn B2', 'STORAGE', 'Dobry', NOW(), NOW()),
(5, 3, 'INV-003-001', 'AVAILABLE', 'PÄ‚Ĺ‚Äąâ€ška SF', 'OPEN_STACK', 'Bardzo dobry', NOW(), NOW()),
(6, 3, 'INV-003-002', 'AVAILABLE', 'PÄ‚Ĺ‚Äąâ€ška SF', 'OPEN_STACK', 'Dobry', NOW(), NOW()),
(7, 8, 'INV-008-001', 'AVAILABLE', 'Cyfrowy', 'REFERENCE', 'Nieaplikowalny', NOW(), NOW()),
(8, 9, 'INV-009-001', 'AVAILABLE', 'PÄ‚Ĺ‚Äąâ€ška dystopie', 'OPEN_STACK', 'Dobry', NOW(), NOW()),
(9, 10, 'INV-010-001', 'BORROWED', 'WypoÄąÄ˝yczona', 'OPEN_STACK', 'Dobry', NOW(), NOW()),
(10, 10, 'INV-010-002', 'AVAILABLE', 'PÄ‚Ĺ‚Äąâ€ška SF', 'OPEN_STACK', 'Bardzo dobry', NOW(), NOW()),
(11, 6, 'INV-006-001', 'RESERVED', 'Rezerwacja', 'OPEN_STACK', 'Ĺšredni', NOW(), NOW()),
(12, 6, 'INV-006-002', 'AVAILABLE', 'Magazyn H1', 'STORAGE', 'Dobry', NOW(), NOW()),
(13, 11, 'INV-011-001', 'AVAILABLE', 'PÄ‚Ĺ‚Äąâ€ška klasyka', 'OPEN_STACK', 'Dobry', NOW(), NOW()),
(14, 12, 'INV-012-001', 'BORROWED', 'WypoÄąÄ˝yczona', 'OPEN_STACK', 'Bardzo dobry', NOW(), NOW()),
(15, 13, 'INV-013-001', 'AVAILABLE', 'PÄ‚Ĺ‚Äąâ€ška literatura', 'OPEN_STACK', 'Dobry', NOW(), NOW()),
(16, 14, 'INV-014-001', 'RESERVED', 'Rezerwacja', 'OPEN_STACK', 'Bardzo dobry', NOW(), NOW()),
(17, 15, 'INV-015-001', 'AVAILABLE', 'PÄ‚Ĺ‚Äąâ€ška SF', 'OPEN_STACK', 'Dobry', NOW(), NOW()),
(18, 16, 'INV-016-001', 'BORROWED', 'WypoÄąÄ˝yczona', 'STORAGE', 'Dobry', NOW(), NOW()),
(19, 17, 'INV-017-001', 'AVAILABLE', 'PÄ‚Ĺ‚Äąâ€ška dystopie', 'OPEN_STACK', 'Bardzo dobry', NOW(), NOW()),
(20, 18, 'INV-018-001', 'MAINTENANCE', 'Konserwacja', 'STORAGE', 'ZuÄąÄ˝yty', NOW(), NOW()),
(21, 19, 'INV-019-001', 'AVAILABLE', 'PÄ‚Ĺ‚Äąâ€ška kryminaÄąâ€š', 'OPEN_STACK', 'Dobry', NOW(), NOW()),
(22, 20, 'INV-020-001', 'BORROWED', 'WypoÄąÄ˝yczona', 'OPEN_STACK', 'Dobry', NOW(), NOW()),
(23, 21, 'INV-021-001', 'AVAILABLE', 'PÄ‚Ĺ‚Äąâ€ška klasyka', 'OPEN_STACK', 'Dobry', NOW(), NOW()),
(24, 22, 'INV-022-001', 'AVAILABLE', 'PĂłĹ‚ka historia', 'OPEN_STACK', 'Ĺšredni', NOW(), NOW()),
(25, 23, 'INV-023-001', 'RESERVED', 'Rezerwacja', 'OPEN_STACK', 'Bardzo dobry', NOW(), NOW()),
(26, 24, 'INV-024-001', 'AVAILABLE', 'PÄ‚Ĺ‚Äąâ€ška SF', 'OPEN_STACK', 'Dobry', NOW(), NOW()),
(27, 27, 'INV-027-001', 'AVAILABLE', 'Cyfrowy', 'REFERENCE', 'Nieaplikowalny', NOW(), NOW()),
(28, 25, 'INV-025-001', 'AVAILABLE', 'PÄ‚Ĺ‚Äąâ€ška fantasy', 'OPEN_STACK', 'Bardzo dobry', NOW(), NOW()),
(29, 26, 'INV-026-001', 'BORROWED', 'WypoÄąÄ˝yczona', 'OPEN_STACK', 'Dobry', NOW(), NOW()),
(30, 30, 'INV-030-001', 'AVAILABLE', 'PÄ‚Ĺ‚Äąâ€ška psychologia', 'OPEN_STACK', 'Dobry', NOW(), NOW());
...
-- Zasoby cyfrowe ksiĂ„â€¦ÄąÄ˝ek (30)
...
(4, 'Spis treÄąâ€şci', 'book_05_spis_treÄąâ€şci.pdf', 'application/pdf', 12950947, 'asset_004_5270c475a5.pdf', NOW(), 5),
(5, 'OkÄąâ€šadka', 'book_06_okÄąâ€šadka.jpg', 'image/jpeg', 6882383, 'asset_005_34ffb55da2.jpg', NOW(), 6),
...
(9, 'Spis treÄąâ€şci', 'book_10_spis_treÄąâ€şci.pdf', 'application/pdf', 14727140, 'asset_009_a01107e3d8.pdf', NOW(), 10),
(10, 'OkÄąâ€šadka', 'book_11_okÄąâ€šadka.jpg', 'image/jpeg', 690112, 'asset_010_dacfe45b1c.jpg', NOW(), 11),
...
(14, 'Spis treÄąâ€şci', 'book_15_spis_treÄąâ€şci.pdf', 'application/pdf', 5623352, 'asset_014_f2b1506c96.pdf', NOW(), 15),
(15, 'OkÄąâ€šadka', 'book_16_okÄąâ€šadka.jpg', 'image/jpeg', 5767645, 'asset_015_deb343d706.jpg', NOW(), 16),
...
(19, 'Spis treÄąâ€şci', 'book_20_spis_treÄąâ€şci.pdf', 'application/pdf', 2315888, 'asset_019_b08044bc3a.pdf', NOW(), 20),
(20, 'OkÄąâ€šadka', 'book_21_okÄąâ€šadka.jpg', 'image/jpeg', 5707556, 'asset_020_68160881ab.jpg', NOW(), 21),
...
(24, 'Spis treÄąâ€şci', 'book_25_spis_treÄąâ€şci.pdf', 'application/pdf', 4163158, 'asset_024_c62b170d2b.pdf', NOW(), 25),
(25, 'OkÄąâ€šadka', 'book_26_okÄąâ€šadka.jpg', 'image/jpeg', 2797719, 'asset_025_fded8d59d3.jpg', NOW(), 26),
...
(29, 'Spis treÄąâ€şci', 'book_30_spis_treÄąâ€şci.pdf', 'application/pdf', 1519910, 'asset_029_ece932ecc4.pdf', NOW(), 30),
(30, 'OkÄąâ€šadka', 'book_01_okÄąâ€šadka.jpg', 'image/jpeg', 14790970, 'asset_030_dd7bb1073d.jpg', NOW(), 1);

-- WypoÄąÄ˝yczenia (30)
INSERT INTO loan (id, book_id, book_copy_id, user_id, borrowed_at, due_at, returned_at, extensions_count, last_extended_at, updated_at) VALUES
(1, 1, 1, 6, NOW() - INTERVAL '9 days', NOW() + INTERVAL '30 days', NULL, 0, NULL, NOW()),
(2, 1, 2, 9, NOW() - INTERVAL '12 days', NOW() + INTERVAL '26 days', NULL, 0, NULL, NOW()),
(3, 1, 3, 12, NOW() - INTERVAL '9 days', NOW() + INTERVAL '18 days', NULL, 1, NOW() - INTERVAL '91 days', NOW()),
(4, 2, 4, 15, NOW() - INTERVAL '14 days', NOW() + INTERVAL '5 days', NULL, 0, NULL, NOW()),
(5, 3, 5, 18, NOW() - INTERVAL '4 days', NOW() + INTERVAL '24 days', NULL, 0, NULL, NOW()),
(6, 3, 6, 21, NOW() - INTERVAL '3 days', NOW() + INTERVAL '21 days', NULL, 1, NOW() - INTERVAL '55 days', NOW()),
(7, 8, 7, 24, NOW() - INTERVAL '23 days', NOW() + INTERVAL '7 days', NULL, 0, NULL, NOW()),
(8, 9, 8, 27, NOW() - INTERVAL '3 days', NOW() + INTERVAL '18 days', NULL, 0, NULL, NOW()),
(9, 10, 9, 30, NOW() - INTERVAL '1 days', NOW() + INTERVAL '30 days', NULL, 1, NOW() - INTERVAL '78 days', NOW()),
(10, 10, 10, 5, NOW() - INTERVAL '13 days', NOW() + INTERVAL '7 days', NULL, 0, NULL, NOW()),
(11, 6, 11, 8, NOW() - INTERVAL '18 days', NOW() + INTERVAL '27 days', NULL, 0, NULL, NOW()),
(12, 6, 12, 11, NOW() - INTERVAL '21 days', NOW() + INTERVAL '17 days', NULL, 1, NOW() - INTERVAL '13 days', NOW()),
(13, 11, 13, 14, NOW() - INTERVAL '11 days', NOW() + INTERVAL '13 days', NULL, 0, NULL, NOW()),
(14, 12, 14, 17, NOW() - INTERVAL '16 days', NOW() + INTERVAL '6 days', NULL, 0, NULL, NOW()),
(15, 13, 15, 20, NOW() - INTERVAL '173 days', NOW() - INTERVAL '26 days', NOW() - INTERVAL '3 days', 1, NOW() - INTERVAL '54 days', NOW()),
(16, 14, 16, 23, NOW() - INTERVAL '36 days', NOW() - INTERVAL '45 days', NOW() - INTERVAL '1 days', 0, NULL, NOW()),
(17, 15, 17, 26, NOW() - INTERVAL '137 days', NOW() - INTERVAL '9 days', NOW() - INTERVAL '1 days', 0, NULL, NOW()),
(18, 16, 18, 29, NOW() - INTERVAL '125 days', NOW() - INTERVAL '47 days', NOW() - INTERVAL '1 days', 1, NOW() - INTERVAL '105 days', NOW()),
(19, 17, 19, 4, NOW() - INTERVAL '50 days', NOW() - INTERVAL '57 days', NOW() - INTERVAL '0 days', 0, NULL, NOW()),
(20, 18, 20, 7, NOW() - INTERVAL '162 days', NOW() - INTERVAL '29 days', NOW() - INTERVAL '4 days', 0, NULL, NOW()),
(21, 25, NULL, 10, NOW() - INTERVAL '108 days', NOW() - INTERVAL '14 days', NOW() - INTERVAL '2 days', 1, NOW() - INTERVAL '109 days', NOW()),
(22, 29, NULL, 13, NOW() - INTERVAL '129 days', NOW() - INTERVAL '28 days', NOW() - INTERVAL '1 days', 0, NULL, NOW()),
(23, 3, NULL, 16, NOW() - INTERVAL '166 days', NOW() - INTERVAL '26 days', NOW() - INTERVAL '4 days', 0, NULL, NOW()),
(24, 7, NULL, 19, NOW() - INTERVAL '60 days', NOW() - INTERVAL '45 days', NOW() - INTERVAL '1 days', 1, NOW() - INTERVAL '112 days', NOW()),
(25, 11, NULL, 22, NOW() - INTERVAL '180 days', NOW() - INTERVAL '29 days', NOW() - INTERVAL '3 days', 0, NULL, NOW()),
(26, 15, NULL, 25, NOW() - INTERVAL '45 days', NOW() - INTERVAL '52 days', NOW() - INTERVAL '1 days', 0, NULL, NOW()),
(27, 19, NULL, 28, NOW() - INTERVAL '146 days', NOW() - INTERVAL '47 days', NOW() - INTERVAL '0 days', 1, NOW() - INTERVAL '54 days', NOW()),
(28, 23, NULL, 3, NOW() - INTERVAL '145 days', NOW() - INTERVAL '60 days', NOW() - INTERVAL '3 days', 0, NULL, NOW()),
(29, 27, NULL, 6, NOW() - INTERVAL '107 days', NOW() - INTERVAL '32 days', NOW() - INTERVAL '0 days', 0, NULL, NOW()),
(30, 1, NULL, 9, NOW() - INTERVAL '70 days', NOW() - INTERVAL '38 days', NOW() - INTERVAL '2 days', 1, NOW() - INTERVAL '76 days', NOW());
...
(1, 11.61, 'PLN', 'OpÄąâ€šata administracyjna', NOW() - INTERVAL '43 days', NULL, 1),
(2, 40.76, 'PLN', 'Uszkodzenie egzemplarza', NOW() - INTERVAL '23 days', NULL, 2),
(3, 19.74, 'PLN', 'Przetrzymanie', NOW() - INTERVAL '53 days', NULL, 3),
(4, 19.39, 'PLN', 'Zgubienie ksiĂ„â€¦ÄąÄ˝ki', NOW() - INTERVAL '22 days', NOW() - INTERVAL '27 days', 4),
(5, 28.39, 'PLN', 'Zgubienie ksiĂ„â€¦ÄąÄ˝ki', NOW() - INTERVAL '8 days', NULL, 5),
(6, 46.16, 'PLN', 'Przetrzymanie', NOW() - INTERVAL '33 days', NULL, 6),
(7, 25.64, 'PLN', 'Zgubienie ksiĂ„â€¦ÄąÄ˝ki', NOW() - INTERVAL '7 days', NULL, 7),
(8, 47.27, 'PLN', 'Przetrzymanie', NOW() - INTERVAL '75 days', NOW() - INTERVAL '12 days', 8),
(9, 15.63, 'PLN', 'OpÄąâ€šata administracyjna', NOW() - INTERVAL '73 days', NULL, 9),
(10, 38.51, 'PLN', 'Zgubienie ksiĂ„â€¦ÄąÄ˝ki', NOW() - INTERVAL '31 days', NULL, 10),
(11, 32.69, 'PLN', 'Zgubienie ksiĂ„â€¦ÄąÄ˝ki', NOW() - INTERVAL '56 days', NULL, 11),
(12, 28.58, 'PLN', 'Zgubienie ksiĂ„â€¦ÄąÄ˝ki', NOW() - INTERVAL '31 days', NOW() - INTERVAL '30 days', 12),
(13, 34.83, 'PLN', 'Zgubienie ksiĂ„â€¦ÄąÄ˝ki', NOW() - INTERVAL '43 days', NULL, 13),
(14, 47.79, 'PLN', 'Uszkodzenie egzemplarza', NOW() - INTERVAL '15 days', NULL, 14),
(15, 1.98, 'PLN', 'OpÄąâ€šata administracyjna', NOW() - INTERVAL '16 days', NULL, 15),
(16, 24.39, 'PLN', 'Przetrzymanie', NOW() - INTERVAL '60 days', NOW() - INTERVAL '17 days', 16),
(17, 7.94, 'PLN', 'Zgubienie ksiĂ„â€¦ÄąÄ˝ki', NOW() - INTERVAL '28 days', NULL, 17),
(18, 30.29, 'PLN', 'Przetrzymanie', NOW() - INTERVAL '59 days', NULL, 18),
(19, 25.7, 'PLN', 'Zgubienie ksiĂ„â€¦ÄąÄ˝ki', NOW() - INTERVAL '90 days', NULL, 19),
...
(22, 14.97, 'PLN', 'OpÄąâ€šata administracyjna', NOW() - INTERVAL '45 days', NULL, 22),
...
(26, 25.49, 'PLN', 'OpÄąâ€šata administracyjna', NOW() - INTERVAL '28 days', NULL, 26),
(27, 14.84, 'PLN', 'Przetrzymanie', NOW() - INTERVAL '13 days', NULL, 27),
(28, 22.69, 'PLN', 'Zgubienie ksiĂ„â€¦ÄąÄ˝ki', NOW() - INTERVAL '24 days', NOW() - INTERVAL '16 days', 28),
(29, 47.1, 'PLN', 'Przetrzymanie', NOW() - INTERVAL '81 days', NULL, 29),
(30, 47.03, 'PLN', 'OpÄąâ€šata administracyjna', NOW() - INTERVAL '57 days', NULL, 30);
...
-- Relacje kolekcja-ksiĂ„â€¦ÄąÄ˝ki (30)
...
(1, 'OgÄąâ€šoszenie #01', 'TreÄąâ€şĂ„â€ˇ ogÄąâ€šoszenia #01 (dane testowe).', 'event', 'draft', true, true, NOW() - INTERVAL '17 days', NOW() - INTERVAL '17 days', NULL, NULL, NOW() + INTERVAL '10 days', '["all"]', 2),
(2, 'OgÄąâ€šoszenie #02', 'TreÄąâ€şĂ„â€ˇ ogÄąâ€šoszenia #02 (dane testowe).', 'maintenance', 'archived', true, true, NOW() - INTERVAL '108 days', NOW() - INTERVAL '108 days', NULL, NULL, NULL, '["all"]', 2),
(3, 'OgÄąâ€šoszenie #03', 'TreÄąâ€şĂ„â€ˇ ogÄąâ€šoszenia #03 (dane testowe).', 'policy', 'published', true, true, NOW() - INTERVAL '110 days', NOW() - INTERVAL '110 days', NOW() - INTERVAL '110 days', NOW() + INTERVAL '10 days', NULL, '["all"]', 2),
(4, 'OgÄąâ€šoszenie #04', 'TreÄąâ€şĂ„â€ˇ ogÄąâ€šoszenia #04 (dane testowe).', 'info', 'draft', false, true, NOW() - INTERVAL '20 days', NOW() - INTERVAL '20 days', NULL, NULL, NULL, '["ROLE_USER"]', 2),
(5, 'OgÄąâ€šoszenie #05', 'TreÄąâ€şĂ„â€ˇ ogÄąâ€šoszenia #05 (dane testowe).', 'event', 'archived', false, true, NOW() - INTERVAL '100 days', NOW() - INTERVAL '100 days', NULL, NULL, NOW() + INTERVAL '13 days', '["all"]', 2),
(6, 'OgÄąâ€šoszenie #06', 'TreÄąâ€şĂ„â€ˇ ogÄąâ€šoszenia #06 (dane testowe).', 'maintenance', 'published', false, true, NOW() - INTERVAL '101 days', NOW() - INTERVAL '101 days', NOW() - INTERVAL '101 days', NOW() + INTERVAL '21 days', NULL, '["ROLE_LIBRARIAN","ROLE_ADMIN"]', 2),
(7, 'OgÄąâ€šoszenie #07', 'TreÄąâ€şĂ„â€ˇ ogÄąâ€šoszenia #07 (dane testowe).', 'policy', 'draft', false, true, NOW() - INTERVAL '42 days', NOW() - INTERVAL '42 days', NULL, NULL, NULL, '["all"]', 1),
(8, 'OgÄąâ€šoszenie #08', 'TreÄąâ€şĂ„â€ˇ ogÄąâ€šoszenia #08 (dane testowe).', 'info', 'archived', false, true, NOW() - INTERVAL '4 days', NOW() - INTERVAL '4 days', NULL, NULL, NULL, '["ROLE_USER"]', 2),
(9, 'OgÄąâ€šoszenie #09', 'TreÄąâ€şĂ„â€ˇ ogÄąâ€šoszenia #09 (dane testowe).', 'event', 'published', false, true, NOW() - INTERVAL '12 days', NOW() - INTERVAL '12 days', NOW() - INTERVAL '12 days', NOW() + INTERVAL '28 days', NOW() + INTERVAL '16 days', '["all"]', 2),
(10, 'OgÄąâ€šoszenie #10', 'TreÄąâ€şĂ„â€ˇ ogÄąâ€šoszenia #10 (dane testowe).', 'maintenance', 'draft', false, true, NOW() - INTERVAL '28 days', NOW() - INTERVAL '28 days', NULL, NULL, NULL, '["all"]', 2),
(11, 'OgÄąâ€šoszenie #11', 'TreÄąâ€şĂ„â€ˇ ogÄąâ€šoszenia #11 (dane testowe).', 'policy', 'archived', false, false, NOW() - INTERVAL '58 days', NOW() - INTERVAL '58 days', NULL, NULL, NULL, '["all"]', 2),
(12, 'OgÄąâ€šoszenie #12', 'TreÄąâ€şĂ„â€ˇ ogÄąâ€šoszenia #12 (dane testowe).', 'info', 'published', false, false, NOW() - INTERVAL '55 days', NOW() - INTERVAL '55 days', NOW() - INTERVAL '55 days', NOW() + INTERVAL '35 days', NULL, '["ROLE_USER"]', 2),
(13, 'OgÄąâ€šoszenie #13', 'TreÄąâ€şĂ„â€ˇ ogÄąâ€šoszenia #13 (dane testowe).', 'event', 'draft', false, false, NOW() - INTERVAL '84 days', NOW() - INTERVAL '84 days', NULL, NULL, NOW() + INTERVAL '19 days', '["all"]', 2),
(14, 'OgÄąâ€šoszenie #14', 'TreÄąâ€şĂ„â€ˇ ogÄąâ€šoszenia #14 (dane testowe).', 'maintenance', 'archived', false, false, NOW() - INTERVAL '51 days', NOW() - INTERVAL '51 days', NULL, NULL, NULL, '["all"]', 1),
(15, 'OgÄąâ€šoszenie #15', 'TreÄąâ€şĂ„â€ˇ ogÄąâ€šoszenia #15 (dane testowe).', 'policy', 'published', false, true, NOW() - INTERVAL '7 days', NOW() - INTERVAL '7 days', NOW() - INTERVAL '7 days', NOW() + INTERVAL '17 days', NULL, '["all"]', 2),
(16, 'OgÄąâ€šoszenie #16', 'TreÄąâ€şĂ„â€ˇ ogÄąâ€šoszenia #16 (dane testowe).', 'info', 'draft', false, false, NOW() - INTERVAL '19 days', NOW() - INTERVAL '19 days', NULL, NULL, NULL, '["ROLE_USER"]', 2),
(17, 'OgÄąâ€šoszenie #17', 'TreÄąâ€şĂ„â€ˇ ogÄąâ€šoszenia #17 (dane testowe).', 'event', 'archived', false, false, NOW() - INTERVAL '14 days', NOW() - INTERVAL '14 days', NULL, NULL, NOW() + INTERVAL '22 days', '["all"]', 2),
(18, 'OgÄąâ€šoszenie #18', 'TreÄąâ€şĂ„â€ˇ ogÄąâ€šoszenia #18 (dane testowe).', 'maintenance', 'published', false, false, NOW() - INTERVAL '59 days', NOW() - INTERVAL '59 days', NOW() - INTERVAL '59 days', NOW() + INTERVAL '21 days', NULL, '["ROLE_LIBRARIAN","ROLE_ADMIN"]', 2),
(19, 'OgÄąâ€šoszenie #19', 'TreÄąâ€şĂ„â€ˇ ogÄąâ€šoszenia #19 (dane testowe).', 'policy', 'draft', false, false, NOW() - INTERVAL '17 days', NOW() - INTERVAL '17 days', NULL, NULL, NULL, '["all"]', 2),
(20, 'OgÄąâ€šoszenie #20', 'TreÄąâ€şĂ„â€ˇ ogÄąâ€šoszenia #20 (dane testowe).', 'info', 'archived', false, true, NOW() - INTERVAL '85 days', NOW() - INTERVAL '85 days', NULL, NULL, NULL, '["ROLE_USER"]', 2),
(21, 'OgÄąâ€šoszenie #21', 'TreÄąâ€şĂ„â€ˇ ogÄąâ€šoszenia #21 (dane testowe).', 'event', 'published', false, false, NOW() - INTERVAL '5 days', NOW() - INTERVAL '5 days', NOW() - INTERVAL '5 days', NOW() + INTERVAL '55 days', NOW() + INTERVAL '25 days', '["all"]', 1),
(22, 'OgÄąâ€šoszenie #22', 'TreÄąâ€şĂ„â€ˇ ogÄąâ€šoszenia #22 (dane testowe).', 'maintenance', 'draft', false, false, NOW() - INTERVAL '56 days', NOW() - INTERVAL '56 days', NULL, NULL, NULL, '["all"]', 2),
(23, 'OgÄąâ€šoszenie #23', 'TreÄąâ€şĂ„â€ˇ ogÄąâ€šoszenia #23 (dane testowe).', 'policy', 'archived', false, false, NOW() - INTERVAL '103 days', NOW() - INTERVAL '103 days', NULL, NULL, NULL, '["all"]', 2),
(24, 'OgÄąâ€šoszenie #24', 'TreÄąâ€şĂ„â€ˇ ogÄąâ€šoszenia #24 (dane testowe).', 'info', 'published', false, false, NOW() - INTERVAL '104 days', NOW() - INTERVAL '104 days', NOW() - INTERVAL '104 days', NOW() + INTERVAL '36 days', NULL, '["ROLE_USER"]', 2),
(25, 'OgÄąâ€šoszenie #25', 'TreÄąâ€şĂ„â€ˇ ogÄąâ€šoszenia #25 (dane testowe).', 'event', 'draft', false, true, NOW() - INTERVAL '104 days', NOW() - INTERVAL '104 days', NULL, NULL, NOW() + INTERVAL '28 days', '["all"]', 2),
(26, 'OgÄąâ€šoszenie #26', 'TreÄąâ€şĂ„â€ˇ ogÄąâ€šoszenia #26 (dane testowe).', 'maintenance', 'archived', false, false, NOW() - INTERVAL '30 days', NOW() - INTERVAL '30 days', NULL, NULL, NULL, '["all"]', 2),
(27, 'OgÄąâ€šoszenie #27', 'TreÄąâ€şĂ„â€ˇ ogÄąâ€šoszenia #27 (dane testowe).', 'policy', 'published', false, false, NOW() - INTERVAL '30 days', NOW() - INTERVAL '30 days', NOW() - INTERVAL '30 days', NOW() + INTERVAL '65 days', NULL, '["all"]', 2),
(28, 'OgÄąâ€šoszenie #28', 'TreÄąâ€şĂ„â€ˇ ogÄąâ€šoszenia #28 (dane testowe).', 'info', 'draft', false, false, NOW() - INTERVAL '43 days', NOW() - INTERVAL '43 days', NULL, NULL, NULL, '["ROLE_USER"]', 1),
(29, 'OgÄąâ€šoszenie #29', 'TreÄąâ€şĂ„â€ˇ ogÄąâ€šoszenia #29 (dane testowe).', 'event', 'archived', false, false, NOW() - INTERVAL '85 days', NOW() - INTERVAL '85 days', NULL, NULL, NOW() + INTERVAL '31 days', '["all"]', 2),
(30, 'OgÄąâ€šoszenie #30', 'TreÄąâ€şĂ„â€ˇ ogÄąâ€šoszenia #30 (dane testowe).', 'maintenance', 'published', false, true, NOW() - INTERVAL '18 days', NOW() - INTERVAL '18 days', NOW() - INTERVAL '18 days', NOW() + INTERVAL '21 days', NULL, '["ROLE_LIBRARIAN","ROLE_ADMIN"]', 2);
...
-- Dostawcy (3)
INSERT INTO supplier (id, name, contact_email, contact_phone, address_line, city, country, tax_identifier, notes, active, created_at, updated_at) VALUES
(1, 'Supplier 01', 'contact01@supplier.pl', '+48111111111', 'Street 1', 'City', 'PL', 'TAX-01', 'Contact 01', true, NOW() - INTERVAL '60 days', NOW() - INTERVAL '10 days'),
(2, 'Supplier 02', 'contact02@supplier.pl', '+48222222222', 'Street 2', 'City', 'PL', 'TAX-02', 'Contact 02', true, NOW() - INTERVAL '90 days', NOW() - INTERVAL '20 days'),
(3, 'Supplier 03', 'contact03@supplier.pl', '+48333333333', 'Street 3', 'City', 'PL', 'TAX-03', 'Contact 03', true, NOW() - INTERVAL '120 days', NOW() - INTERVAL '30 days');
